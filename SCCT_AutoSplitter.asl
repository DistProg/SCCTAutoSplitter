...
{
    // Load removal
    bool inMenu: "splintercell3.exe", 0x00A0E3DC, 0x8, 0x94;
    ...
    bool missionEnd: "splintercell3.exe", 0x008FA4E8, 0xE8;
...
start
{
    if (settings["il_mode"] && settings["il_noql"]) {
        ...
        // Start when the Lighthouse cutscene starts playing
        return current.mapName == "01_Lighthouse" && !old.cutscene && current.cutscene;
    }
}
...
gameTime {
    ...
    } else if (!vars.inLevel || current.inMenu || current.mapName == "menu") {
